...
pub mod pallet {
	...
	use sp_io::hashing::blake2_128; //生成128位值 //生成随机值
	...
	impl<T: Config> Pallet<T> {
		...
		pub fn breed(
			...
			// 	new_dna[i] = (selector[i] & kitty_1.0[i]) | (!selector[i] & kitty_2.0[i]);
			// }
			let kitty = Kitty { name, dna: Self::child_kitty_dna(&who, &kitty_1, &kitty_2) };
